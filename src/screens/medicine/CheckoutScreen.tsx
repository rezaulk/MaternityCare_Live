...
const CheckoutScreen = ({ navigation }: { navigation: any }) => {
  ...
  const [totalorders, setTotalOrders] = React.useState<Orders[]>([]);
  ...
  //     .catch((err) => { err && console.log(err); });
    ...
    if (currentOrder != null) {
      debugger;
      let  _currentOrder: Orders = JSON.parse(currentOrder);
      
      
      let totalOrder = await getData("@TotalOrder");
      if (totalOrder != null) {
        debugger;
        let _address: Orders[] = JSON.parse(totalOrder);
  
          _address.push(_currentOrder)
       
        await storeData("@TotalOrder",JSON.stringify(_address));
      }  
      else{
        let _address: Orders[] = [];
        _address.push(_currentOrder);
